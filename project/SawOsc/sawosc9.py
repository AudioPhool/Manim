...
class anim(Scene):
    def construct(self):
        ...
        capscope = SineScope(lambda x: ((signal.sawtooth(5*x + phi.get_value())*0.1)+0.9), a[1], RIGHT, 0.2, GREEN, 0.7)
        outscope = SineScope(lambda x: comp(signal.sawtooth(5*x + phi.get_value()+0.2)), a[4], RIGHT, 7.7, ORANGE, 0.7)

        v1 = always_redraw(lambda: DecimalNumber(((signal.sawtooth(5*4.29 + phi.get_value())*0.1)+0.9)*5, 1).next_to(capscope.ax, UP, buff = 0.25))
        v1l = always_redraw(lambda: Tex("V").scale(0.8).next_to(v1,RIGHT,0.05))

        v2 = always_redraw(lambda: DecimalNumber(comp2d((signal.sawtooth(5*4.29 + phi.get_value()-0.1)*0.1)+0.9)*15, 1).next_to(outscope.ax, UP, buff = 0.25).shift(LEFT*2.25))
        v2l = always_redraw(lambda: Tex("V").scale(0.8).next_to(v2,RIGHT,0.05))

        self.play(FadeIn(a, capscope.group, outscope.group))
        self.wait(0.2)#1
        self.play(outscope.ax.animate.shift(UP*0.75))
        self.wait(0.2)#3

        v2 = always_redraw(lambda: DecimalNumber(comp2d((signal.sawtooth(5*4.29 + phi.get_value()-0.1)*0.1)+0.9)*15, 1).next_to(outscope.ax, UP, buff = 0.25))
        v2l = always_redraw(lambda: Tex("V").scale(0.8).next_to(v2,RIGHT,0.05))

        self.play(FadeIn(v5, v1,v1l,v2,v2l))
        self.wait(0.2)#5
        ...
        (OA.+) to[short] ++(-0.5,0);""",#0
        r"""\draw (0.5, 2.25) to[C] (3.5,2.25) ;""",#1
        r"""\draw (0.5, 2.25) to[short](0.5, 3.25) to[short] (1.25,3.25) node[njfet, rotate=-90, anchor=S](Q1){} (Q1.G)to[C, l_ = $C_{gs}$](0.5, 3.25) (Q1.D)to[short](3.5,3.25) to[short] (3.5,2.25) (Q1.G)to[short] ++(5,0)to[diode]++(0,-1)node[ground]{} ;""",#1
        r"""\draw (0.5,0.025) to[short] (0.5,-0.5) node[ground]{} ;""",#2
        r"""\draw (-1.5,-0.5) node[ground]{} to[vsource](-1.5,1) to[R] (0.5, 1);""",#3
