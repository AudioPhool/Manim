...
class anim(Scene):
    def construct(self):
        ...
        capscope2 = SineScope(lambda x: (signal.sawtooth(3*x + phi.get_value())*0.5)+0.5, [4,0.7,0], RIGHT, 0.2, GREEN, 0.7)
        outscope2 = SineScope(lambda x: comp(signal.sawtooth(3*x + phi.get_value()-0.1)), [4,0.7,0], RIGHT, 0.2, ORANGE, 0.7)
