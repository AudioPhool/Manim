...
class anim(Scene):
    def construct(self):
        ...
        vpo = ValueTracker(1)
        freq = ValueTracker(220)
        keyb = ImageMobject("project/MIDI/key.jpg").shift(LEFT*0+UP*0).scale(1)
        hl1 = Rectangle(color=RED,fill_color = RED_A, fill_opacity = 0.85, height=1.75, width=0.4).shift(LEFT*2.9+DOWN*1.05)

        volt = always_redraw(lambda: DecimalNumber(number=vpo.get_value()).scale(2).shift(RIGHT*3.5+DOWN*1))
        labv = MathTex('v').scale(2).next_to(volt, RIGHT, buff=0.2)
        freq = always_redraw(lambda: DecimalNumber(number=freq.get_value(), num_decimal_places=0).scale(2).shift(RIGHT*3.5+DOWN*2))
        labf = MathTex('Hz').scale(2).next_to(freq, RIGHT, buff=0.2)
        scope1 = SineScope(lambda x: np.sin((3 * x * vpo.get_value())), volt, UP, 1.25,YELLOW)

        self.play(FadeIn(keyb))
        self.wait() 
        self.play(keyb.animate.shift(LEFT*2).scale(0.75),FadeIn(volt, labv, freq, labf), FadeIn(scope1.group),run_time=1)
        self.wait()
        self.play(FadeIn(hl1))
        self.wait()
        self.play(vpo.animate.set_value(2), freq.animate.set_value(440), hl1.animate.shift(RIGHT*3.15))
        self.wait()
        self.play(hl1.animate.shift(LEFT*3.15))
        self.wait()
        self.play(vpo.animate.set_value(3), freq.animate.set_value(880), hl1.animate.shift(RIGHT*3.15))
        self.wait()
        self.play(hl1.animate.shift(LEFT*3.15))
        self.wait()
        self.play(vpo.animate.set_value(4), freq.animate.set_value(1760), hl1.animate.shift(RIGHT*3.15))
