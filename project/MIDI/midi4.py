...
class anim(Scene):
    def construct(self):
        ...
        self.play(FadeIn(keyb), Create(kin), uart1_rx_label.animate.shift(LEFT*1.25))
        self.wait()#5
        ...
        rxne_flg = Tex("RXNE", color = GOLD).scale(0.75).move_to(nvic.get_center()+np.array([0,-0.25,0]))

        self.play(rxne_flg.animate.move_to(cpu.get_center()+np.array([0,0.25,0])))
        self.play(rxne_flg.animate.set_opacity(0))
        self.wait()#25

        cpurd = Line(start = cpu.get_left(), end = uart1.get_right()+np.array([0,-0.35, 0])).set_z_index(-1)

        self.play(Create(cpurd))
        self.play(binary_byte.animate.move_to(cpu.get_center()+np.array([0,-0.25, 0]))) 
        self.play(FadeOut(binary_byte))

        self.wait()#29

        binary_digits = [0, 0, 1, 1, 0, 0, 0, 0]
        decimal_digits = [DecimalNumber(digit, num_decimal_places=0) for digit in binary_digits]
        decimal_digits = VGroup(*decimal_digits)
        decimal_digits.move_to(kin.get_start() + [0, 0.25, 0])
        ...
        self.play(rxne_flg.animate.set_opacity(0))
        self.play(binary_byte.animate.move_to(cpu.get_center()+np.array([0,-0.25, 0]))) 
        self.play(FadeOut(binary_byte), uart1_rx_label.animate.shift(RIGHT*1.25))
